...
export default function Editor({ project: p, reload }: {
    ...
    const keyFor=(cid:string)=>draftKey(session.id,p.id,cid);
    ...
    },[dirty,chapter?.id,chapter?.version,title,text,recovery,session.id]);
    ...
    const load = async (id: string) => { const serial = ++requestId.current; setLoading(true); try {
        ...
        try{local=readDraft(localStorage,keyFor(c.id));if(!local&&session.email){const oldKey=draftKey(session.email,p.id,c.id);local=readDraft(localStorage,oldKey);if(local&&writeDraft(localStorage,keyFor(c.id),local))removeDraft(localStorage,oldKey)}if(local&&local.title===c.title&&local.content===c.content){removeDraft(localStorage,keyFor(c.id));local=null}}catch{}
    ...
    async function save() { if (!chapter || busy || loading || recovery)
