...
export default function Sources({ project: p, reload }: {
    ...
    return <><div className="section-top"><div><h2>Fuentes que puedes volver a encontrar.</h2><p>Guarda texto, conserva su procedencia y busca los pasajes que necesitas.</p></div><button className="btn" onClick={() => setOpen(true)}><Plus size={17}/>Añadir fuente</button></div><div className="info-banner"><FileText size={20}/><p>Importa archivos <b>TXT o Markdown en UTF-8</b>, o pega un fragmento. Cada fuente conserva su texto completo y se divide en pasajes para buscar. Hasta 500.000 caracteres por fuente.</p></div><form className="filter-bar" onSubmit={async (e) => { e.preventDefault(); setSearching(true); try {
