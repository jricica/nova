...
function ProfileForm({ initial }: {
    ...
    <form onSubmit={async (e) => { e.preventDefault(); setBusy(true); setError(''); try {
      ...
      </fieldset><aside className="profile-rail"><section><h2>Sesión y acceso</h2><h3>Cuenta de NOVA</h3><p className="account-email">{session.username || session.email || session.displayName}</p><p>Administra tu usuario, contraseña y recuperación desde Seguridad.</p><a className="help-link" href="/seguridad">Configurar mi acceso<ArrowRight size={16}/></a><LogoutButton className="logout-link"/></section><section><h2>Tu identidad de escritura</h2><p>Las muestras, el tono y los criterios de estilo se configuran en Mi voz.</p><a className="help-link" href="/mi-voz">Configurar mi voz<ArrowRight size={16}/></a></section></aside></div>
